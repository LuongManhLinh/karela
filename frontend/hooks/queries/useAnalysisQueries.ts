...
export const ANALYSIS_KEYS = {
  ...
  summaries: (connectionId: string) => [...ANALYSIS_KEYS.all, "summaries", connectionId] as const,
  details: (analysisId: string) => [...ANALYSIS_KEYS.all, "details", analysisId] as const,
  statuses: (ids: string[]) => [...ANALYSIS_KEYS.all, "statuses", ...ids.sort()] as const,
};
...
export const useAnalysisDetailsQuery = (analysisId: string | null) => {
  return useQuery({
    queryKey: ANALYSIS_KEYS.details(analysisId || ""),
    queryFn: () => analysisService.getAnalysisDetails(analysisId!),
    enabled: !!analysisId,
  });
};
...
export const useRunAnalysisMutation = () => {
    ...
    return useMutation({
        mutationFn: ({ connectionId, projectKey, data }: { connectionId: string; projectKey: string; data: RunAnalysisRequest }) =>
            analysisService.runAnalysis(connectionId, projectKey, data),
        onSuccess: (_, variables) => {
            queryClient.invalidateQueries({ queryKey: ANALYSIS_KEYS.summaries(variables.connectionId) });
        },
    });
};
...
export const useRerunAnalysisMutation = () => {
    ...
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (analysisId: string) => analysisService.rerunAnalysis(analysisId),
...
export const useGenerateProposalsMutation = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (analysisId: string) => analysisService.generateProposalsFromAnalysis(analysisId),
         onSuccess: (_, analysisId) => {
             ...
             queryClient.invalidateQueries({ queryKey: ["proposals", "session", analysisId] });
        }
    });
};
