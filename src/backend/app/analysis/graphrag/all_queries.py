...
MATCH path = (s1:Entity {type: 'KEY', bucket: $bucket})-[:RELATED {type: 'DEPENDS_ON'}|:RELATED {type: 'BLOCKS'}]->(s2:Entity {type: 'KEY'})-[:RELATED {type: 'DEPENDS_ON'}|:RELATED {type: 'BLOCKS'}]->(s1)
...
WHERE NOT exists((s)-[:RELATED*1..2]-(:Entity {type: 'ROLE', bucket: $bucket}))
...
WHERE complexity_node.type IN ['SYSTEM', 'COMPONENT', 'DATA', 'RULE']
