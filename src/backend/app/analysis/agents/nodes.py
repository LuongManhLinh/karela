...
def build_pairwise_defect_agent(targeted: bool = False):
    if targeted:
...
def run_context_gatherer(
    ...
    if target_story:
        ...
        message = f"Gather project-level context relevant to the following target story:\n{story_text}"
    else:
        message = (
...
def run_dependency_matrix_analyzer(
    ...
    output: DependencyMatrixResponse = None

    try:
        output = response["structured_response"]
    except Exception as e:
        print(f"| ERROR parsing DependencyMatrixResponse: {e}")

    if not output:
        print(f"| Try to parse last message")
        output = parse_last_message(response, Clazz=DependencyMatrixResponse)

    if not output:
        print(
            f"| Failed to parse DependencyMatrixResponse. Returning empty defect list."
        )
        return []
