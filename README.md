...
Sau đó mở file [src/backend/.env](src/backend/.env) và điền giá trị.

Các biến cần chú ý:

- `LLM_PROVIDER`: chọn `gemini` hoặc `openai`.
- `GEMINI_API_KEYS`: điền khi `LLM_PROVIDER=gemini`.
- `OPENAI_API_KEYS`: điền khi `LLM_PROVIDER=openai`.
- `MINERU_TOKEN`: cần thiết để chuyển đổi PDF sang Markdown
...
3. Chọn app vừa tạo.
4. Mở mục **Settings**.
5. Copy `client id` và `secret` vào đúng hai biến dưới đây trong file `src/backend/.env`:
...
Điền hai biến dưới đây trong file `src/backend/.env` bằng domain thật của bạn:
...
#### Cách 1: Dùng Docker tự build backend

Cách này phù hợp nếu bạn muốn chỉnh code backend rồi chạy, nhưng vẫn muốn tận dụng Docker để tránh phải cài nhiều thứ trên máy.
